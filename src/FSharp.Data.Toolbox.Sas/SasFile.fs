    ...
    //                Mix 
            ...
            let readMeta () =
                ...
                let subHeaderPointerBytes n = 
                    ...
                    page.[totalOffset .. totalOffset + 2 * header.WordLength + 1]                

                // process_subheader_pointers
                let subHeaderPointer (bytes: byte array) =
                ...
                let subHeader subHeaderPointer =
                    let subHeader =
                        if subHeaderPointer.Compression = TRUNCATED_SUBHEADER_ID ||
                           subHeaderPointer.Length = 0 then Truncated 
                        else
                ...
                [0..subHeaderCount - 1] 
                |> List.map subHeaderPointerBytes 
                |> List.map subHeaderPointer
                |> List.map subHeader
            ...
            match pageType with
    //        | PAGE_META_TYPE -> readMeta() //|> Meta
    //        | PAGE_DATA_TYPE -> [] |> Meta
            | PAGE_MIX_TYPE1
            | PAGE_MIX_TYPE2 -> readMeta() 
